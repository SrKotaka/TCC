...
def get_city_coordinates(city_name):
    ...
    try:
        ...
        if location:
            return {"nome": city_name, "lat": location.latitude, "lon": location.longitude}
    except Exception as e:
        print(f"Error fetching {city_name}: {e}")
    return None

def generate_city_json(city_list):
    cities_data = []
    for city in city_list:
        result = get_city_coordinates(city)
        if result:
            cities_data.append(result)
        time.sleep(1)  # Add a 1-second delay to respect Nominatim's rate limit
    return json.dumps(cities_data, ensure_ascii=False, indent=4)
...
"Zortéa (SC)"
    ...
    json_output = generate_city_json(cidades)
    print(json_output)